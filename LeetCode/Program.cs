...
static bool HasGroupsSizeX(int[] deck)
{
    ...
    var numCntDic = new Dictionary<int, int>();
    //遍历数组
    foreach (var value in deck)
    {
        //字典键中是否包含当前数，包含，即该数已存在，则加一，不包含，即不存在，则赋值为1
        numCntDic[value] = numCntDic.ContainsKey(value) ? numCntDic[value] + 1 : 1;
    }

    //3:找到字典值集合中的最小值
    var minCnt = numCntDic.Values.Min();
    //如果最小值小于二，则返回false
    if (minCnt < 2)
        return false;
    ...
    for (var i = 1; i <= Math.Sqrt(minCnt); i++)
    {
        //取余等于0
        if (minCnt % i == 0)
        {
    ...
    //遍历所有其他值b
    foreach (var curCnt in numCntDic.Values)
    {
        //相同不用计算
        if (curCnt != minCnt)
        {
